          ...
          <template v-else>
            <div class="header">
              ...
              <div class="name">
                ...
                <n-gradient-text type="error" class="subtitle" :depth="3">{{ subtitle }}</n-gradient-text>
              </div>
              <div class="data">
                <n-text
                  v-if="listData.total"
...
const listType = ref(
  router.currentRoute.value.params.type || store.newsArr[0].value
);

const news = store.newsArr.find((element) => element.value === listType.value);

const label = ref(news?.label || '');

const subtitle = ref(news?.subtitle || '');
...
watch(
  ...
  () => {
    if (listData.value) {
      updateTime.value = formatTime(listData.value.requestTime);
    }
  }
);
...
watch(
  ...
  (val) => {
    if (val.fullPath !== '/') {
      listType.value = val.params.type;
      const news = store.newsArr.find((element) => element.value === listType.value);
      if (news) {
        label.value = news.label;
        subtitle.value = news.subtitle;
